class tab extends tabBoundary {
  ...
  boolean border,open,parentCanvas,overflow,docking,docked;
...
void setCanvas(tab t){
  ...
  void logic() {
    ...
    if(draggable){
      ...
      if(drag){
        if(BMS.currentMouseObject==null)BMS.currentMouseObject = itemLabel;
        if(BMS.currentObject==null)BMS.currentObject = this;
        x = mouseX+dx;
        y = mouseY+dy;

        if(BMS.dock.pos(new PVector(x,y))&&!docking)docking = true;
        //else docking = false;
      }
