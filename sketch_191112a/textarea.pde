class TextArea{
  ...
  void getKey(){
    ...
    if(!copy_clipboard&&clipboard_toggle==0){
        ...
        if(a!=null&&timer%delay==0&&!exclusion()&&key!=BACKSPACE&&(current_width+ l.w<totalwidth)){
          ...
          if(hindex==-1){
            ...
            if((hindex!=textbox.size()-1||hindex!=-1)&&timer%delay==0){
        ...
        else if(keyPressed && key==BACKSPACE&&timer%delay==0){delete();}
    }
    else if(copy_clipboard&&clipboard_toggle==1&&timer%delay==0&&!tsize){
